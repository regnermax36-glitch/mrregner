...
# Run extraction and patching (only once, prevent duplicates)
if [ -z "$MAXREGNEROS_V3_PROCESSED" ]; then
    export MAXREGNEROS_V3_PROCESSED="true"
    
    LOG_BEGIN "Running extraction and patching tools"

    # Extract and redesign icons (this replaces Generate-Icons.sh)
    ...
    LOG_INFO "maxregnerOS v3 already processed, skipping"
fi
